<template>
    <div class="container">
        <div class="game-header">
            <h1 class="title">Key Clash (Personal Game Room)</h1>
...
onMounted(() => {
    ...
    // Inside your client-side component where you're handling socket events

    socket.on('inviteAccepted', ({ roomId, players }) => {
        // Redirect the sender to the invite clash page
        console.log('other person accepted')
        router.push(`/inviteclash?roomId=${roomId}&players=${players.join(',')}`);
    });
