  ...
  setup() {
    ...
    const fetchGameStats = async (gameName) => {
      ...
      try {
        ...
        console.log(`User Scores for ${gameName}:`, userScores);
        console.log(`Average Scores for ${gameName}:`, averageScores);
        ...
        gameData.value[gameName] = formattedUserScores;
    ...
    const chartData = computed(() => {
      ...
      games.forEach((gameName) => {
        ...
        const labels = userScores.map(entry => entry.create_at.toLocaleDateString());

        // Generate datasets
        const userScoresData = userScores.map(entry => entry.score);
        const avgScoresData = averageScores.map(entry => entry.avg_score);
...
<template>
  <div class="performance-chart-container">
    ...
    <div class="charts-grid">
      <div 
        v-for="(gameName, index) in games" 
        :key="gameName" 
        class="game-chart"
      >
        <h3 class="game-title">{{ gameName }}</h3>
        <div v-if="chartData[gameName]" class="chart-container">
